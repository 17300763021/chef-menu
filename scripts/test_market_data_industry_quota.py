...
class IndustryQuotaGuardTest(unittest.TestCase):
    def test_fresh_storage_capacity_below_80_is_allowed(self) -> None:
        result = evaluate_industry_quota(
            storage_percent="73.2%", checked_at="2026-07-28", now=NOW,
        )
    ...
    def test_invalid_or_future_attestation_fails_closed(self) -> None:
        for storage, checked_at in (
            ...
            with self.assertRaises(RuntimeError):
                evaluate_industry_quota(
                    storage_percent=storage, checked_at=checked_at, now=NOW,
                )
    ...
    def test_workflow_is_manual_bounded_resumable_and_node24_pinned(self) -> None:
        ...
        self.assertIn("secrets.MYSQL_PASSWORD", workflow)
        ...
        self.assertNotIn("MYSQL_HOST: gateway", workflow)
