...
TRANSIENT_ERROR_CODES = {"10002007"}
...
class BaostockHistorySource:
    ...
    def _query_rows(self, factory: Callable[[], Any], label: str) -> list[dict[str, str]]:
        ...
        for attempt in range(1, self.attempts + 1):
            ...
            try:
                ...
                code = str(query.error_code)
                try:
                    rows = self._rows(query, label)
                except OSError as error:
                    last_error = error
                    retryable = True
                    ...
                    code = str(query.error_code)
                    last_error = error
                    retryable = code in TRANSIENT_ERROR_CODES
                    if code in BLACKLIST_ERROR_CODES or not retryable:
                        raise
                else:
                    return rows
            if not retryable:
        ...
        if isinstance(last_error, RuntimeError):
            raise last_error
        raise RuntimeError(f"BaoStock {label} failed after {self.attempts} attempts: {last_error}") from last_error
    ...
    def fetch_status(self, symbol: str, start: date, end: date) -> dict[date, dict[str, str]]:
        ...
        return {parse_date(row["date"]): row for row in rows}
    ...
    def fetch_adjusted_prices(self, symbol: str, start: date, end: date, adjustflag: str) -> dict[date, AdjustedOHLC]:
        ...
        rows = self._query_rows(
            ...
            f"adjusted prices {code} adjustflag={adjustflag}",
        )
    ...
    def fetch_reference(self, symbol: str) -> SecurityReference:
        code = normalize_symbol(symbol)
        rows = self._query_rows(
            lambda: self._bs.query_stock_basic(code=baostock_symbol(code)), f"reference {code}",
        )
    ...
    def fetch_adjustments(self, symbol: str, end: date) -> list[AdjustmentEvent]:
        ...
        rows = self._query_rows(
            lambda: self._bs.query_adjust_factor(
                code=baostock_symbol(code), start_date="1990-01-01", end_date=end.isoformat(),
            ),
            f"adjustments {code}",
        )
        return [
