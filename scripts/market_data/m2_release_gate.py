...
CREATE TABLE IF NOT EXISTS m2_data_releases (
  ...
  UNIQUE KEY uq_m2_release_business_date (business_date)
)
"""
...
def build_release(connection: Any, business_date: date) -> dict[str, Any]:
    with connection.cursor() as cursor:
        ...
        industry = _one(cursor, """SELECT dataset_id,as_of_date,authoritative,simulation_orders_allowed,manifest_sha256
            FROM m2_industry_runs WHERE mode='full' AND accepted=1 ORDER BY as_of_date DESC LIMIT 1""")
        fundamental = _one(cursor, """SELECT dataset_id,as_of_date,authoritative,simulation_orders_allowed,manifest_sha256
        ...
        daily = _one(cursor, """SELECT dataset_id,target_session,authoritative,simulation_orders_allowed,manifest_sha256
        ...
        flow = _one(cursor, """SELECT dataset_id,business_date,data_available,authoritative,simulation_orders_allowed,manifest_sha256
            FROM m2_flow_runs WHERE boundary_accepted=1 ORDER BY business_date DESC LIMIT 1""")
    component_rows = {
        "history": {"dataset_id": str(history[0]), "through": str(history[1]), "manifest_sha256": str(history[4])},
        "industry": {"dataset_id": str(industry[0]), "through": str(industry[1]), "manifest_sha256": str(industry[4])},
        "fundamental": {"dataset_id": str(fundamental[0]), "through": str(fundamental[1]), "manifest_sha256": str(fundamental[4])},
        "index": {"dataset_id": str(index[0]), "through": str(index[1]), "manifest_sha256": str(index[4])},
        "daily": {"dataset_id": str(daily[0]), "through": str(daily[1]), "manifest_sha256": str(daily[4])},
        "flow": {"dataset_id": str(flow[0]), "through": str(flow[1]), "data_available": bool(flow[2]), "manifest_sha256": str(flow[5])},
    }
    unsafe = [name for name, row in (("history", history), ("industry", industry), ("fundamental", fundamental), ("index", index), ("daily", daily)) if bool(row[2]) or bool(row[3])]
